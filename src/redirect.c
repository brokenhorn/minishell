...
//	while (info->command->flag == S1 || info->command->flag == S2 ||
//		   info->command->flag == B1 || info->command->flag == B2)
//	{
//		file = info->command->argv[0];
...
//	info->command = info->command->next;
//	if (info->command->flag == S1 || info->command->flag == S2 ||
//			info->command->flag == B1 || info->command->flag == B2)
//	{
//		close(info->fd_redirect);
//		redirect(info);
